...
// Preis in Cent (Integer-Rechnung, um Fließkomma-Rundungsfehler bei Geldbeträgen zu vermeiden).
// Einzelner Festpreis (kein Frühbucher-/Regulärpreis-Tier wie beim DEEP DESIGN Workshop,
// keine Rabattcodes) – 99,00 € netto, glatt.
const PRICE_NET_CENTS = 9900;
...
exports.handler = async (event) => {
  ...
  let priceNetCents = PRICE_NET_CENTS;
  ...
  const metadata = {
    product: "vibe_coding_experimentiertage",
  ...
  try {
    const payment = await mollieRequest("POST", "/payments", {
      ...
      description: "Vibe-Coding Experimentiertage",
      redirectUrl: `${BASE_URL}/vibe-coding-checkout/danke.html`,
      webhookUrl:  `${BASE_URL}/.netlify/functions/mollie-webhook-vibe-coding`,
      metadata,
      locale: "de_DE"
    });
